<template>
       ...
       <el-table-column label="设备" min-width="150">
        <template #default="{ row, $index }">
          <el-form-item :prop="`${$index}.deviceId`" :rules="formRules.deviceId" class="mb-0px!">
            <el-input v-model="row.deviceId" placeholder="请输入设备ID" />
          </el-form-item>
        </template>
      </el-table-column>
  ...
  <el-row justify="center" class="mt-3">
    <el-button @click="handleAdd" round>+ 添加设备</el-button>
  </el-row>
</template>
...
watch(
  ...
  async (val) => {
    ...
    try {
      ...
      formData.value = await MaintenOrderApi.getMaintenOrderDeviceListByWorkOrderId(val)
    } finally {
